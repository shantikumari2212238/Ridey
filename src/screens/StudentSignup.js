...
const BACKEND_URL = "https://rydy-backend.onrender.com/api/students";
...
const StudentSignup = ({ navigation }) => {
  ...
  const validateAndSubmit = async () => {
    if (!name.trim() || !email.trim() || !universityId.trim() || !universityName.trim() || !password.trim()) {
    ...
    try {
      ...
      const resp = await fetch(`${BACKEND_URL}/signup`, { method: "POST", body: formData });
      ...
      // On success server returns student id & email — navigate to OTP screen
      const studentId = json.student?.id || json.student?._id;
      if (!studentId) {
        Alert.alert("Signup", "Signup created. Please verify via email (OTP).");
        navigation.navigate("StudentLogin");
        return;
      }

      Alert.alert("Signup created", "OTP sent to email. Please verify.");
      navigation.replace("StudentOtp", { studentId, email });
    } catch (err) {
      setLoading(false);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <Text style={styles.subtitle}>Register — you'll receive an OTP to verify your email</Text>
      ...
      <View style={styles.form}>
        ...
        <TextInput style={styles.input} value={universityId} onChangeText={setUniversityId} placeholder="2021-ABC-123" placeholderTextColor="#a68bc6" autoCapitalize="none" />
        <Text style={[styles.label, { marginTop: 12 }]}>University Name</Text>
        <TextInput style={styles.input} value={universityName} onChangeText={setUniversityName} placeholder="Your University" placeholderTextColor="#a68bc6" />
        ...
        <TouchableOpacity style={styles.uploadBox} onPress={onChooseImage}>
          {idImage ? <Image source={{ uri: idImage.uri }} style={styles.idPreview} /> : <Text style={styles.uploadText}>Tap to upload ID card</Text>}
        </TouchableOpacity>
